...
class AgentLoop:
    ...
    async def _process_message(
        ...
        # Trigger memory consolidation if session is getting large
    ...
    async def _consolidate_memory(self, session: Session, archive_all: bool = False) -> None:
        ...
        Args:
            ...
            archive_all: If True, archive all messages (for /new command).
                         If False, only process messages beyond the window.
        """
        ...
        if archive_all:
            ...
            keep_count = 0
            logger.info(
                f"Memory consolidation (archive_all): "
                f"{len(session.messages)} total messages archived"
            )
        else:
        ...
        prompt = (
            "You are a memory consolidation agent. Process this conversation and return "
            "a JSON object with exactly two keys:\n\n"
            '1. "history_entry": A paragraph (2-5 sentences) summarizing the key '
            "events/decisions/topics. Start with a timestamp like [YYYY-MM-DD HH:MM]. "
            "Include enough detail to be useful when found by grep search later.\n\n"
            '2. "memory_update": The updated long-term memory content. Add any new facts: '
            "user location, preferences, personal info, habits, project context, technical "
            "decisions, tools/services used. If nothing new, return the existing content "
            "unchanged.\n\n"
        ...
        try:
            ...
            if not isinstance(result, dict):
                logger.warning(
                    f"Memory consolidation: unexpected response type, skipping. "
                    f"Response: {text[:200]}"
                )
                return
